...
public class UserController {
    ...
    public String publishTreasure(Treasure treasure, MultipartFile photo, HttpServletRequest request) {
        ...
        if(treasure.getFare() == null){
            treasure.setFare(0.0);
        }
        treasure.setTotal(treasure.getPrice() + treasure.getFare());
        ...
        try {
            ...
            if ((fileName = uploadService.uploadFile(photo)) != null) {
                ...
                if (treasureService.treasurePublish(treasure)) {
                    request.setAttribute("tid", treasure.getId());
    ...
    public Object uploadPhoto(MultipartFile photo, HttpSession session) {
        ...
        try {
            ...
            if (user.getPhoto() != null
                    && !Constant.MALE_PHOTO_DEFAULT.equals(user.getPhoto())
                    && !Constant.FEMALE_PHOTO_DEFAULT.equals(user.getPhoto())) {
                if (!uploadService.deleteFile(user.getPhoto())) {
                    return "{\"result\":\"false\"}";
                }
            }
            //获取用户头像
            log.debug("文件名称====>" + photo.getName());
        ...
        return "{\"result\":\"true\",\"photo\":\"" + uploadPhotoPath + "\"}";
